...
export default function NavigationMenu({ onClose }: NavigationMenuProps) {
  const menuItems = [
    { 
      title: "Cosa Facciamo", 
      subItems: ["Serramenti", "Arredamento Design", "Carpenteria", "Manutenzioni"],
      ...
      <div className="flex-1 pt-40 pb-20 px-10 md:px-24 overflow-y-auto">
                    ...
                    <div className="hidden lg:flex flex-col gap-2 pt-4">
                      {item.subItems.map((sub) => (
                        <a
                          key={sub}
                          href="#"
                          className="text-[1.6rem] text-dark/60 hover:text-red transition-colors"
                        >
                          {sub}
                        </a>
                      ))}
        ...
        <div className="mt-20 flex flex-col gap-8">
          <div className="flex flex-col gap-2">
            ...
            <a href={`mailto:${CONTACT_INFO.email}`} className="text-[1.6rem] md:text-[1.8rem] text-red multi-line-underline">
              {CONTACT_INFO.email}
            </a>
          </div>
          <a href="#" className="btn btn-dark w-fit">
